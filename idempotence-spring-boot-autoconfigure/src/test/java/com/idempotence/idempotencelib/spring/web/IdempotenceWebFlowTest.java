...
class IdempotenceWebFlowTest {
    ...
    void shouldReturnConflictForSameKeyButDifferentBody() throws Exception {
        ...
        mockMvc.perform(post("/orders").contentType(APPLICATION_JSON)
                .header("Idempotency-Key", key).content("{\"amount\": 100}"))
                .andExpect(status().isCreated());

        mockMvc.perform(post("/orders").contentType(APPLICATION_JSON)
                .header("Idempotency-Key", key).content("{\"amount\": 200}"))
    ...
    void shouldReturnBadRequestWhenIdempotencyKeyIsMissing() throws Exception {
        mockMvc.perform(post("/orders").contentType(APPLICATION_JSON).content("{\"amount\": 100}"))
